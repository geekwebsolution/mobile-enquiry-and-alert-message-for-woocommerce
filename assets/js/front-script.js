...
jQuery(document).ready(function() {
    ...
    jQuery("body").on("show_variation", ".variations_form", function() {
        ...
        if(jQuery(".variations tbody tr").length > 0) {
            jQuery(".variations tbody tr").each(function() {
                // store selection of variations
                variable = jQuery(this).find(".label label").text() + ":- " + jQuery(this).find(".value select option:selected").text();
                final_variable += "%0D%0A" + variable;
